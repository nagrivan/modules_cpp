...
void    Bureaucrat::signForm(Form &document) const {
    try
    {
        if (document.getSigned() == false) // возможно, не нужно
        {
            document.beSigned(*this);
            std::cout << this->_name << " signs document " << document.getName() << std::endl;
        }
    }
    catch(const std::exception& e)
    {
        std::cout << this->_name << " can't sign " << document.getName() << e.what() << std::endl;
    }
}
